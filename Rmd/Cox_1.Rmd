...
# Comparaison des 4 premiers gènes
...
```{r}
gene.code="MYC"
m.all<-linearCox.analyze(gene.code)$m
m.mutes<-linearCox.analyze(gene.code,mutations = T)$m
m.non.mutes<-linearCox.analyze(gene.code,mutations = F)$m
...
plot(rna,residuals(m.all,type="martingale"),main=paste0("Martingale residuals according to RNA level of ",gene.code),ylab="martingale residuals",col='white')
